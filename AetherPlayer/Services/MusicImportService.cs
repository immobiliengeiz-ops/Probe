...
public class MusicImportService : IMusicImportService
{
    ...
    {
        if (string.IsNullOrWhiteSpace(filePath) || !File.Exists(filePath) || !IsSupportedAudioFile(filePath))
        {
            return null;
        }
    ...
    private static Track BuildTrackFromFile(string audioPath)
    {
        ...
        try
        {
            ...
            return new Track
            {
                ...
                MoodTags = ["Imported"],
                Duration = duration,
                AddedOn = File.GetCreationTime(audioPath),
                Lyrics = lyrics,
                AudioPath = audioPath,
                CoverPath = coverPath,
                IsFavorite = false,
    ...
    private static string? ExtractCoverArt(TagLib.File media, string audioPath)
    {
        ...
        {
            System.IO.File.WriteAllBytes(file, picture.Data.Data);
        }
