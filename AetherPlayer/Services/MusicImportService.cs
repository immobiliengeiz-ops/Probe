...
public class MusicImportService : IMusicImportService
{
    ...
    public IReadOnlyList<Track> LoadFromFolder(string folderPath, bool recursive = true)
    {
        ...
        var files = EnumerateAudioFilesSafe(folderPath, recursive)
    ...
    private static IEnumerable<string> EnumerateAudioFilesSafe(string rootFolder, bool recursive)
    {
        if (!recursive)
        {
            ...
            try
            {
                topFiles = Directory.EnumerateFiles(rootFolder);
            }
            catch
            {
                // ignored
