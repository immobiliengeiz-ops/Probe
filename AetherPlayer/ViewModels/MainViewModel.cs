...
public class MainViewModel : ViewModelBase
{
    ...
    public MainViewModel()
    {
        ...
        PreviousCommand = new RelayCommand(_ => PlayPrevious(), _ => TracksView.Cast<TrackViewModel>().Any());
        _nextCommand = new RelayCommand(_ => PlayNext(), _ => QueueTracks.Count > 0 || TracksView.Cast<TrackViewModel>().Any());
    ...
    {
        var visible = TracksView.Cast<TrackViewModel>().ToList();
        if (visible.Count == 0)
        {
            return;
        }

        var current = SelectedTrack is not null ? visible.IndexOf(SelectedTrack) : 0;
        var previous = current <= 0 ? visible[^1] : visible[current - 1];
        PlayTrack(previous);
    }
    ...
    private void PlayNext()
    {
        ...
        if (RepeatMode == RepeatMode.One && SelectedTrack is not null)
        {
        ...
        if (ShuffleEnabled)
        {
            next = visible[_random.Next(visible.Count)];
        }
        else
        {
            var current = SelectedTrack is not null ? visible.IndexOf(SelectedTrack) : -1;
            if (current >= visible.Count - 1)
            {
                next = RepeatMode == RepeatMode.All ? visible[0] : visible[^1];
            }
            else
            {
        ...
        if (next.IsMissing)
        {
            var fallback = visible.FirstOrDefault(t => !t.IsMissing);
    ...
    private void PickLibraryFolder()
    {
        ...
        {
            Description = "Select a music folder",
    ...
    private void RefreshFilterOptions()
    {
        Genres.Clear();
        Genres.Add("All");
        foreach (var genre in _allTracks.Select(t => t.Genre).Distinct().OrderBy(g => g))
        {
            Genres.Add(genre);
        }
        ...
        Moods.Clear();
        Moods.Add("All");
        foreach (var mood in _allTracks.SelectMany(t => t.Model.MoodTags).Distinct().OrderBy(m => m))
        {
            Moods.Add(mood);
        }
    }
