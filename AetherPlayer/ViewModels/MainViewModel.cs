...
public class MainViewModel : ViewModelBase
{
    ...
    private const string RootSubfolderLabel = "(Root)";
    ...
    private string _importStatusMessage = "No music folders imported yet.";
    ...
    public MainViewModel()
    {
        ...
        UserCollectionsView = CollectionViewSource.GetDefaultView(UserCollections);
        ...
        AddSelectedToPlaylistCommand = new RelayCommand(_ => AddSelectedToCurrentPlaylist(), _ => SelectedTracks.Count > 0 && SelectedPlaylist is not null);
        RenamePlaylistCommand = new RelayCommand(param => RenamePlaylist(param as PlaylistViewModel));
        DeletePlaylistCommand = new RelayCommand(param => DeletePlaylist(param as PlaylistViewModel));
        ...
        RenameCollectionCommand = new RelayCommand(param => RenameCollection(param as UserCollection));
        DeleteCollectionCommand = new RelayCommand(param => DeleteCollection(param as UserCollection));
        ...
        RemoveSelectedFromPlaylistCommand = new RelayCommand(_ => RemoveSelectedFromPlaylist(), _ => SelectedTracks.Count > 0 && SelectedPlaylist is not null);
    ...
    public ICollectionView UserCollectionsView { get; }
    ...
    public RelayCommand AddSelectedToPlaylistCommand { get; }
    public RelayCommand RenamePlaylistCommand { get; }
    ...
    public RelayCommand RemoveSelectedFromPlaylistCommand { get; }
    ...
    public UserCollection? SelectedCollection
    {
        ...
        RemoveSelectedFromPlaylistCommand.RaiseCanExecuteChanged();
            }
        }
    }
    ...
    public PlaylistViewModel? SelectedPlaylist
    {
        ...
        set
        {
            ...
            {
                OnPropertyChanged(nameof(SelectedPlaylistTracks));
                RemoveSelectedFromPlaylistCommand.RaiseCanExecuteChanged();
                AddSelectedToPlaylistCommand.RaiseCanExecuteChanged();
            }
        }
    }
    ...
    public int ImportedTrackCount => _allTracks.Count;
    public string ImportStatusMessage => _importStatusMessage;
    ...
    public IReadOnlyList<TrackViewModel> SelectedPlaylistTracks =>
        SelectedPlaylist is null
            ? []
            : _allTracks.Where(t => SelectedPlaylist.Model.Tracks.Any(p => p.Id == t.Model.Id)).ToList();

    public double CurrentPositionSeconds
    {
        get => _isUserSeeking ? _pendingSeekSeconds : _audioService.Position.TotalSeconds;
        set
        {
            if (_isUserSeeking)
            {
                _pendingSeekSeconds = Math.Clamp(value, 0, CurrentDurationSeconds);
                OnPropertyChanged();
                OnPropertyChanged(nameof(CurrentPositionDisplay));
                return;
            }
    ...
    public string CurrentPositionDisplay
    {
        get
        {
            var position = _isUserSeeking ? TimeSpan.FromSeconds(_pendingSeekSeconds) : _audioService.Position;
            return $"{(int)position.TotalMinutes}:{position.Seconds:D2}";
        }
    }
    ...
    private bool FilterTracks(object item)
    {
        ...
        var trackMainFolder = ResolveTrackMainFolder(track);
        var matchesGenre = SelectedGenreFilter == "All"
            || string.Equals(trackMainFolder, SelectedGenreFilter, StringComparison.OrdinalIgnoreCase);
        var matchesMood = SelectedMoodFilter == "All" || MatchesSubfolderFilter(track, trackMainFolder);
    ...
    {
        var orderedVisible = TracksView.Cast<TrackViewModel>()
            .Where(t => !t.IsMissing)
            .ToList();
    ...
    public void BeginSeekInteraction()
    {
        ...
        _isUserSeeking = true;
        _pendingSeekSeconds = _audioService.Position.TotalSeconds;
        _resumeAfterSeek = _audioService.IsPlaying;
        if (_resumeAfterSeek)
        {
            _audioService.Pause();
        }
    ...
    public void UpdateSeekPreview(double seconds)
    {
        ...
        _pendingSeekSeconds = Math.Clamp(seconds, 0, CurrentDurationSeconds);
        OnPropertyChanged(nameof(CurrentPositionSeconds));
        OnPropertyChanged(nameof(CurrentPositionDisplay));
    }

    public void CommitSeekInteraction(double seconds)
    {
        if (!_isUserSeeking)
        {
            SeekTo(seconds);
        ...
        _pendingSeekSeconds = Math.Clamp(seconds, 0, CurrentDurationSeconds);
        SeekTo(_pendingSeekSeconds);
        _isUserSeeking = false;

        if (_resumeAfterSeek && CurrentTrack is not null)
        {
            _audioService.Play();
        }
    ...
    {
        var clamped = Math.Clamp(seconds, 0, CurrentDurationSeconds);
        _audioService.Seek(TimeSpan.FromSeconds(clamped));
        OnPropertyChanged(nameof(CurrentPositionSeconds));
        OnPropertyChanged(nameof(CurrentPositionDisplay));
        OnPropertyChanged(nameof(CurrentDurationDisplay));
    }
    ...
    private void AddToQueue(TrackViewModel? track)
    {
        ...
        {
            QueueTracks.Insert(insertIndex, target);
            insertIndex++;
        }
    ...
    private void AddSelectedToCurrentPlaylist()
    {
        if (SelectedPlaylist is null || SelectedTracks.Count == 0)
        {
            return;
        }
        ...
        foreach (var track in SelectedTracks)
        {
            if (SelectedPlaylist.Model.Tracks.All(x => x.Id != track.Model.Id))
            {
                SelectedPlaylist.Model.Tracks.Add(track.Model);
            }
        }

        SelectedPlaylist.NotifyTrackCollectionChanged();
        OnPropertyChanged(nameof(SelectedPlaylistTracks));
        SaveAppState();
    }
    ...
    private void AddToPlaylist(TrackViewModel? track)
    {
        ...
        playlistVm.NotifyTrackCollectionChanged();
        SelectedPlaylist = playlistVm;
        OnPropertyChanged(nameof(SelectedPlaylistTracks));
        RemoveSelectedFromPlaylistCommand.RaiseCanExecuteChanged();
        AddSelectedToPlaylistCommand.RaiseCanExecuteChanged();
        SaveAppState();
    ...
    private void RenamePlaylist(PlaylistViewModel? playlist)
    {
        ...
        var existing = Playlists
            .Where(p => !ReferenceEquals(p, target))
            .Select(p => p.Name);

        var nextName = _selectionDialogService.ChoosePlaylist(existing)?.Trim();
        if (string.IsNullOrWhiteSpace(nextName) || target.Name.Equals(nextName, StringComparison.OrdinalIgnoreCase))
        {
            return;
        }

        target.Model.Name = nextName;
        target.NotifyHeaderChanged();
        SaveAppState();
    }
    ...
    private void DeletePlaylist(PlaylistViewModel? playlist)
    {
        ...
        var removedIndex = Playlists.IndexOf(target);
        if (removedIndex < 0)
        {
            return;
        }
        ...
        if (ReferenceEquals(SelectedPlaylist, target))
        {
            SelectedPlaylist = Playlists.ElementAtOrDefault(Math.Clamp(removedIndex - 1, 0, Math.Max(0, Playlists.Count - 1)));
        }
    ...
    private void RenameCollection(UserCollection? collection)
    {
        ...
        var existing = UserCollections
            .Where(c => !ReferenceEquals(c, target))
            .Select(c => c.Name);

        var nextName = _selectionDialogService.ChooseCollection(existing)?.Trim();
        if (string.IsNullOrWhiteSpace(nextName) || target.Name.Equals(nextName, StringComparison.OrdinalIgnoreCase))
        {
            return;
        }
    ...
    private void DeleteCollection(UserCollection? collection)
    {
        var target = collection ?? SelectedCollection;
        if (target is null)
        {
        ...
        var removedIndex = UserCollections.IndexOf(target);
        if (removedIndex < 0)
        {
            return;
        }

        UserCollections.RemoveAt(removedIndex);
        if (ReferenceEquals(SelectedCollection, target))
        {
            SelectedCollection = UserCollections.ElementAtOrDefault(Math.Clamp(removedIndex - 1, 0, Math.Max(0, UserCollections.Count - 1)));
        }
    ...
    private void RemoveFromLibrary(TrackViewModel? track)
    {
        ...
        foreach (var target in targets)
        {
            ...
            {
                var before = playlist.Model.Tracks.Count;
                playlist.Model.Tracks.RemoveAll(t => t.Id == target.Model.Id);
                if (playlist.Model.Tracks.Count != before)
                {
                    playlist.NotifyTrackCollectionChanged();
                }
            }
        }
    ...
    private void ImportFolder(string folderPath)
    {
        ...
        if (imported.Count == 0)
        {
            _importStatusMessage = $"No supported audio files found in '{folderPath}'.";
        ...
        _importStatusMessage = addedCount > 0
            ? $"Imported {addedCount} track(s) from '{folderPath}'."
            : $"Folder '{folderPath}' scanned: no new tracks added.";

        RefreshFilterOptions();
        ApplySort();
        RefreshTracksView();
        OnPropertyChanged(nameof(ImportedTrackCount));
        OnPropertyChanged(nameof(ImportStatusMessage));
    }
    ...
    private void RemoveSelectedFromPlaylist()
    {
        ...
        var selectedIds = SelectedTracks.Select(t => t.Model.Id).ToHashSet();
        var before = SelectedPlaylist.Model.Tracks.Count;
        SelectedPlaylist.Model.Tracks.RemoveAll(t => selectedIds.Contains(t.Id));
        if (SelectedPlaylist.Model.Tracks.Count != before)
        {
            SelectedPlaylist.NotifyTrackCollectionChanged();
        }
        OnPropertyChanged(nameof(SelectedPlaylistTracks));
    ...
    private void RefreshFilterOptions()
    {
        ...
        if (!Genres.Contains(_selectedGenreFilter))
        {
            _selectedGenreFilter = "All";
    ...
    private void RefreshSubfolderOptions()
    {
        ...
        var subfolders = _allTracks
            .Select(track =>
            {
                var mainFolder = ResolveTrackMainFolder(track);
                if (string.IsNullOrWhiteSpace(mainFolder))
                {
                    return null;
                }

                if (!SelectedGenreFilter.Equals("All", StringComparison.OrdinalIgnoreCase)
                    && !mainFolder.Equals(SelectedGenreFilter, StringComparison.OrdinalIgnoreCase))
                {
                    return null;
                }

                var subfolder = ResolveTrackSubfolder(track, mainFolder);
                return SelectedGenreFilter.Equals("All", StringComparison.OrdinalIgnoreCase)
                    ? BuildSubfolderLabel(mainFolder, subfolder)
                    : subfolder;
            })
        ...
        if (!Moods.Contains(_selectedMoodFilter))
        {
    ...
    private bool MatchesSubfolderFilter(TrackViewModel track, string trackMainFolder)
    {
        if (string.IsNullOrWhiteSpace(trackMainFolder))
        {
            return false;
        }

        var trackSubfolder = ResolveTrackSubfolder(track, trackMainFolder);
        if (SelectedGenreFilter.Equals("All", StringComparison.OrdinalIgnoreCase))
        {
            return string.Equals(
                BuildSubfolderLabel(trackMainFolder, trackSubfolder),
                SelectedMoodFilter,
                StringComparison.OrdinalIgnoreCase);
        }

        return string.Equals(trackSubfolder, SelectedMoodFilter, StringComparison.OrdinalIgnoreCase);
    }

    private string ResolveTrackMainFolder(TrackViewModel track)
    {
        return LibraryFolders
            .Where(folder => track.Model.AudioPath.StartsWith(folder, StringComparison.OrdinalIgnoreCase))
            .OrderByDescending(folder => folder.Length)
            .FirstOrDefault() ?? string.Empty;
    }

    private static string ResolveTrackSubfolder(TrackViewModel track, string mainFolder)
    {
        var trackDirectory = Path.GetDirectoryName(track.Model.AudioPath) ?? string.Empty;
        if (string.IsNullOrWhiteSpace(trackDirectory)
            || !trackDirectory.StartsWith(mainFolder, StringComparison.OrdinalIgnoreCase))
        {
            return RootSubfolderLabel;
        }

        var relative = Path.GetRelativePath(mainFolder, trackDirectory)
            .Replace('\\', '/');
    ...
    private static string BuildSubfolderLabel(string mainFolder, string subfolder)
    {
        var mainFolderName = Path.GetFileName(mainFolder.TrimEnd(Path.DirectorySeparatorChar, Path.AltDirectorySeparatorChar));
        if (string.IsNullOrWhiteSpace(mainFolderName))
        {
            mainFolderName = mainFolder;
        }

        return $"{mainFolderName} / {subfolder}";
    }
    ...
    {
        _importStatusMessage = LibraryFolders.Count == 0
            ? "No music folders imported yet."
            : $"{LibraryFolders.Count} folder(s) scanned, {ImportedTrackCount} track(s) available.";
        OnPropertyChanged(nameof(ImportedTrackCount));
        OnPropertyChanged(nameof(ImportStatusMessage));
    }
